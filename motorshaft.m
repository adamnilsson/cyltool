%% 
% Example script for analyzing the motor shaft coupling on t3
...
ring1 = element(1e-3, 4e-3, steel)
ring2 = element(4e-3 - 0e-6, 8e-3, steel)
ring3 = element(8e-3 + 10e-6, 10e-3, steel)
% ring4 = element(21e-3 - 5e-6, 22e-3, carb)
% ring5 = element(22e-3, 31e-3, steel)
rings = [ring1, ring2]

mu = [0.1, 0.1, 0.1, 0.1]        % Friction coefficient
h = 10e-3               % Height of the coupling
...
for i=1:1
    ...
    C2 = (rings(2).ri + rings(2).ry)/(2*E2)*rho2*omega^2;
    grip = Ri - ry
    % Enforce correct deformations
    Fn(2) = C1*Ri - 1/8*C2*Ri^3 + grip;
    Kn(2, [1, 2, 3, 4]) = [ry 1/ry -Ri -1];
        
    % Stress
    Kn(3, 2*i-1:2*i) = E1/(1-v1^2)*[(1+v1) (v1-1)/ry^2]
    Kn(3, 2*i+1:2*i+2) = E2*[-1 0]
end
...
for i=2:1
    ...
    Kn(2*i, 2*i-1:2*i+2) = [ry 1 -Ri -1/Ri]
    
    Kn(2*i+1, 2*i-1:2*i) = E1*[1 0]
...
% Apply preassure on outside
P = -1e6
Fn(2*nr) = (3+rings(nr).matrl.v)/8*rings(nr).ry^2*rings(nr).matrl.rho*omega^2 + P
